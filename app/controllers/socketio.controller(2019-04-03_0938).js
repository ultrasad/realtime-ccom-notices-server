    ...
    socket.on('members room', function(current_room) {
        ...
        if(!userRoomMembers[current_room.group_hash] || Object.keys(userRoomMembers[current_room.group_hash]).length === 0){
            ...
            try {
                ...
                //var usersGroups = ChatService.getUsersGroups({'groups.group_hash':current_room.group_hash}, page=0, limit=50);
                //console.log('usersGroups => ' + usersGroups);

                var callUsersGroupsPromise = async () => {
                    var result = await (ChatService.getUsersGroups({'groups.group_hash':current_room.group_hash}, page=0, limit=50));
                    return result;
                ...
                callUsersGroupsPromise().then(function(usersGroups) {
                    ...
                    //console.log('usersGroups => ' + result);
                    //console.log(util.inspect(result));

                    usersGroups.forEach(function(user) {
                        //console.log('user group => ' + user.username);
                ...
                */
                ...
                //console.log(util.inspect(usersGroups));
                

            } catch(e){
                console.error('members error', { status: 400, message: e.message });
                //return res.status(400).json({ status: 400, message: e.message });
            }

            /* 2019-04-02 move to services
