const UsersModel = require('mongoose').model('Users'); //call module mongoose, Chat model
...
const GroupsModel = require('mongoose').model('Groups'); //call module mongoose, Chat model
...
module.exports = function(io){
    ...
    var call_update_room = function(group, username, uID){
        console.log('call update room, group id => ' + group.group_id + ', username => ' + username + ', uID => ' + uID);

        var callBroadcasePromise = async () => {
            const findBroadcastMessage = SocketIOService.findBroadcastMessage({"groups.group_hash":group.group_hash});
            const findGroupChatMessage = SocketIOService.findGroupChatMessage({"group":group.group_id});

            const [broadcastMessage, logGroupChat] = await Promise.all([findBroadcastMessage, findGroupChatMessage]);

            //return broadcastMessage;
            //console.log('broadcastMessage =>', broadcastMessage);
            //console.log('logGroupChat =>', logGroupChat);

            /*
            for (var logs of logGroupChat){
                console.log('logs msg => ' + logs.fromUser.displayName  + ' | msg | ' + logs.text);
            }
            */
            ...
            for(var user_socket of userSocket[username]) {
                ...
                //typeof socket_user==='string' ? children : children.join('')

                console.log('typeof socket_user => ', typeof socket_user);
                ...
                if(typeof socket_user !== undefined){
                    ...
                    //2019-04-07, old message groups
                    socket_user.emit('log old messages', logGroupChat, group.group_name);

                    //update room msg to user
                    if(broadcastMessage){
                        socket_user.emit('broadcast room message', {message: broadcastMessage.message, priority:broadcastMessage.priority});
                    } else {
                        //2019-03-28, emit hide broadcast msg
                        console.log('nobroadcast msg, hide old broadcast msg >>>');
                    }
                } else {
        ...
        callBroadcasePromise().then(function(chatgroup){
            console.log('call broadcast and chat groups => ', chatgroup);
        }).catch(function(err){
            console.log('call broadcast and chat groups err =>', err.message);
        });

        /*
        SocketIOService.findBroadcastMessage({"groups.group_hash":group.group_hash}).then(function(broadcast){
            //console.log('findBroadcastMessage messages => ' + broadcast);
            for(var user_socket of userSocket[username]) {
                //console.log('user socket id => ' + user_socket + ', username => ' + username + ', new room => ' + group.group_name);
                var socket_user = io.sockets.connected[user_socket];
                ...
                //update room msg to user
                if(broadcast){
                    socket_user.emit('broadcast room message', {message: broadcast.message, priority:broadcast.priority});
                } else {
                    //2019-03-28, emit hide broadcast msg
                    console.log('nobroadcast msg, hide old broadcast msg >>>');
                }
            ...
            console.log('findBroadcastMessage error msg response => ' + error.message);
    ...
    socket.on('add user', (username) => {
        ...
        var callUsersPromise = async () => {
            var userPromise = await (SocketIOService.getUser({username:username}));
            return userPromise;
        };
    ...
    socket.on('members room', function(current_room) {
        ...
        if(!userRoomMembers[current_room.group_hash] || Object.keys(userRoomMembers[current_room.group_hash]).length === 0){
            console.log('userRoomMembers does not exists, group name : '  + current_room.group_name);
            var member_in_room = [];
    ...
    */
    ...
    socket.on('switchRoom', function(group) {
       ...
       if(group.group_type == 'direct'){
           ...
           var new_switch_group = {group_id:group.group_id, group_name:group.group_name, group_hash: group.group_hash, group_type: group.group_type};
           call_update_room(new_switch_group, socket.username, socket.userid);
           console.log('switch groups => ' + new_switch_group);

            //2019-04-08, old solution
            /*
